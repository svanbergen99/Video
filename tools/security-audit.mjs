...
const sensitiveContentPatterns = [
  ...
  ["hard-coded bearer token", /\bBearer\s+[A-Za-z0-9._~+\/-]{24,}\b/i],
...
const dangerousCodePatterns = [
  ...
  ["sync shell/process execution", /\b(?:execSync|spawnSync|execFileSync)\b/],
...
async function walk(dir) {
  ...
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (info.isSymbolicLink()) { findings.push(`${relative(ROOT, absolute)}: symlinks are forbidden`); continue; }
...
function isTextCandidate(path) {
  return /(?:^|\/)(?:[^/]+\.(?:js|mjs|cjs|json|md|txt|ya?ml|html|css|toml)|\.gitignore|\.dockerignore|Dockerfile)$/i.test(path);
}
...
const requiredFragments = [
  ...
  "pathname === \"/healthz\"",
  "requireInternal(request, response)",
  "MAX_JSON_BYTES = 64 * 1024"
];

try {
  const serverText = await readFile(join(ROOT, "server.mjs"), "utf8");
  for (const fragment of requiredFragments) if (!serverText.includes(fragment)) findings.push(`server.mjs: required hardening missing: ${fragment}`);
  if (/readFile\s*\(|createReadStream\s*\(|serveStatic|express\.static|sendFile\s*\(/.test(serverText)) findings.push("server.mjs: file-serving primitive is forbidden");
} catch {
...
for (const absolutePath of files) {
  ...
  if (info.size > MAX_BYTES) { findings.push(`${path}: text file exceeds ${MAX_BYTES} byte limit`); continue; }
  const buffer = await readFile(absolutePath);
  if (buffer.includes(0)) { findings.push(`${path}: unexpected NUL/binary content`); continue; }
...
if (Object.keys(packageJson.dependencies ?? {}).length || Object.keys(packageJson.devDependencies ?? {}).length) findings.push("package.json: runtime must remain zero-dependency");
