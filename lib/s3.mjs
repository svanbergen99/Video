...
export function storageConfig(prefix, env = process.env) {
  ...
  if (configured && !/^https:\/\//i.test(config.endpoint)) {
    throw new Error(`${prefix}_ENDPOINT must use https`);
  }
  if (!new Set(["virtual", "path"]).has(config.urlStyle)) {
    throw new Error(`${prefix}_URL_STYLE must be virtual or path`);
  }
...
export async function putJson(config, key, value) {
  ...
  const response = await fetch(presignObject(config, { method: "PUT", key, expiresSeconds: 300 }), {
    method: "PUT",
...
export async function streamCopy(sourceConfig, sourceKey, targetConfig, targetKey) {
  const source = await fetch(presignObject(sourceConfig, { method: "GET", key: sourceKey, expiresSeconds: 900 }), {
    method: "GET",
    redirect: "error",
  ...
  const target = await fetch(presignObject(targetConfig, { method: "PUT", key: targetKey, expiresSeconds: 900 }), {
    method: "PUT",
    headers,
    body: source.body,
    duplex: "half",
