...
export async function deepQc(url, durationSeconds = 0) {
  ...
  const { stderr } = await runTool("ffmpeg", args, 180_000);
  return { scannedSeconds: maxSeconds, ...parseDetections(stderr) };
}

export function technicalQc(metadata, deep = null, env = process.env) {
  const minimumWidth = Math.max(1, Number(env.MIN_VIDEO_WIDTH || 1280));
  const minimumHeight = Math.max(1, Number(env.MIN_VIDEO_HEIGHT || 720));
  ...
  if (metadata.video && metadata.video.width < minimumWidth) errors.push("width_below_minimum");
  if (metadata.video && metadata.video.height < minimumHeight) errors.push("height_below_minimum");
  ...
  return {
    ...
    requirements: { minimumWidth, minimumHeight, minimumFps },
    errors,
    warnings
  };
}
